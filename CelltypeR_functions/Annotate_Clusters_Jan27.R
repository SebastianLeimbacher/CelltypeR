...
plot_lab_clust <- function(seu, cluster, lables){
  t.lables <- as.data.frame(table(seu.clusters, seu.lable))
...
get_annotation <- function(seu, cluster, lables, top_n = 3, ignore_unknown = FALSE){
  t.lables <- as.data.frame(table(seu.clusters, seu.lable))
  t.lables$Freq <- as.double(t.lables$Freq)
  colnames(t.lables) <- c("Cluster","Lable","Freq")
