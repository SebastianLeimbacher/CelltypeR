...
```{r}
df.input <- read.csv("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/testingLibrary/df1000.csv")
...
pheno.test <-  explore_param(seu, #for phenograph and louvain only
                          ...
                          pheno_lou_kn = c(300,500), #kn for phenograph or louvain
...
lou.test <- explore_param(input = seu, #for phenograph and louvain only
                          ...
                          df_input = df.input, #needed  if input "flowsom"
                          flow_k = NULL, #k for flowsom
                          pheno_lou_kn = c(60), #kn for phenograph or louvain
                          lou_resolution = c(0,0.25,0.5), #resolution for louvain
                          run.plot = TRUE, #print if run
                          run.stats = TRUE, #print and return if run
                          save_to = NULL)

# not - adjust font size in heatmaps
...
# rename Rand_index as clust_stability
