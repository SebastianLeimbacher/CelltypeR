...
rf <- readRDS("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/June9MBOsub_trainedRFM.Rds")
...
seu.r$labels
Idents(seu.r) <- "labels"
rfm.pred.ref <- RFM_predict(seu.r, rf, markers)
head(rfm.pred.ref)

rfm.pred <- RFM_predict(seu.q, rf)
head(rfm.pred)

seu.r <- AddMetaData(object=seu.r, metadata=rfm.pred.ref$`predict(rf, df)`, col.name = 'rfm.labels')

# check that the data is added 
table(seu.r$rfm.labels,seu.r$labels)

# save the results
write.csv(results.df, paste(outpath, "RFMpredictionsAllcells.28072023.csv"))
...
#saveRDS(seu.q, paste(outpath,"All9MOannaote.12072022.Rds",sep=""))
