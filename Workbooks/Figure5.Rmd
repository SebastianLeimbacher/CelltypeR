...
Adjust UMAP parameters and plot again with alpha 0.5

```{r}

unique(seu.all$Celltypes)


seu.all <- ScaleData(seu.all)
seu.all <- RunPCA(seu.all, features = AB, dims = 1:12)
seu.all <- RunUMAP(seu.all, dims = 1:12, n.neighbors = 60, a = 0.7, b = 0.75)


DimPlot(seu.all, group.by = "Celltypes",label = TRUE, raster = FALSE, label.size = 4)
DimPlot(seu.all, group.by = "Batch",label = TRUE, raster = FALSE, label.size = 4)

colnames(seu.all@meta.data)
unique(seu.all$ipsc)
unique(seu.all$Batch)
unique(seu.all$orig.ident)
