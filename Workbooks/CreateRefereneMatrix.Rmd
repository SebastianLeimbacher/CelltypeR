...
input_path <- "/GITHUB/CelltypeR/ExampleOuts/Flow_csv/"

# this is the complete data 
# this data is available in the github

fc <- read.csv(paste(input_path,"2Dcells_surfaceflowset.csv", sep = ""))
...
heatmap(fcz, 
        ...
        Colv = NA,  # Don't cluster columns
        col = colorRampPalette(c("white", "blue"))(100),  # Define a color palette
        main = "Heatmap of Normalized Matrix - FC from 2D cultures")
...
write.csv(fcz, "FC_ReferenceMatrix.csv")


```

Make a heat map of the FC only matrix

```{r}
fc <- read.csv("/Users/rhalenathomas/GITHUB/CelltypeR/ExampleOuts/ReferenceMatrices/FC_ReferenceMatrix.csv")
# add markers as rownames
rownames(fc) <- fc$X
# remove X and reorder
reference_data <- fc %>% select("Astrocyte","IPSC","NPC","Neuron","Oligo","OPC")

#reference_data <- reference_data %>% select(-"X")
#colnames(reference_data) <- c("Astrocyte","Endothelial","Epithelial","Neuron",    "NPC","OPC","Oligo","RG","Stem")
...
# If you need to reorder the levels of the 'Marker' column, you can do it like this
long_df$Marker <- factor(long_df$Marker, levels = marker.order)
...
hm + ggtitle("Expression by cell type from Flow Cytometry") +
  theme(plot.title = element_text(hjust = 0.5))

pdf("ReferenceMatrixFConly.pdf", width = 6.5, height = 5)
hm + ggtitle("Expression by cell type from Flow Cytometry") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()
...
table(seu$Celltypes,seu$Celltypes)
...
scBrain$Stem <- rowMeans(scBrain[, c("IPC","dividing")], na.rm = TRUE)
...
heatmap(RNAbrain, 
        ...
        Colv = NA,  # Don't cluster columns
        col = colorRampPalette(c("white", "blue"))(100),  # Define a color palette
        main = "Heatmap of Normalized Matrix - Brain RNA expression")

# save the average RNA brain expression matrix

write.csv(RNAbrain,"/Users/rhalenathomas/GITHUB/CelltypeR/ExampleOuts/BrainRNAReferenceMatrix.csv")
...
so <- readRDS("/Users/rhalenathomas/Documents/Data/scRNAseq/PublicData/UCSCorg30000cellsSeurat.RDS")
...
heatmap(organoid, 
        ...
        Colv = NA,  # Don't cluster columns
        scale = "column",
        col= colorRampPalette(brewer.pal(8, "Blues"))(25),  # Define a color palette
        main = "Organoid RNA expression ")
legend(x="bottomright", legend=c("min", "ave", "max"), 
     fill=colorRampPalette(brewer.pal(8, "Blues"))(3))
# Add continuous color scale legend
legend(x = "bottomright", 
       legend = c("0","0.5","1"),  # Adjust legend to match the data range
       fill = colorRampPalette(c("white", "blue"))(100),
...
input_path <- "/Users/rhalenathomas/GITHUB/CelltypeR/ExampleOuts/ReferenceMatrices/"
...
# Assign the column names from the reference data frame to the normalized matrix
trans_df_RNA <- zero.one

# quick visualization
heatmap(trans_df_RNA, 
        Rowv = NA,  # Don't cluster rows
        Colv = NA,  # Don't cluster columns
        col = colorRampPalette(c("white", "blue"))(100),  # Define a color palette
        main = "RNA brain and Organoid Normalized Expression")



```

Plot the combined transcriptomics data

```{r}
write.csv(trans_df_RNA,"/Users/rhalenathomas/GITHUB/CelltypeR/ExampleOuts/ReferenceMatrices/RNAReferenceMatrix.csv")

rna <- read.csv("/Users/rhalenathomas/GITHUB/CelltypeR/ExampleOuts/ReferenceMatrices/RNAReferenceMatrix.csv")
# add markers as rownames
rownames(rna) <- rna$X
# remove X and reorder
reference_data <- rna %>% select("Astrocytes","Endothelial","Epithelial","NPC","Neurons","Oligo","OPC","RG","StemCell")

colnames(reference_data) <- c("Astrocyte","Endothelial","Epithelial","NPC","Neuron","Oligo","OPC","RG","Stem")

rownames(reference_data) <-  c("AQP4",  "CD24",  "CD44", "CD184", "CD15", "HepaCAM", "CD29", "CD56",  "O4", "CD140a","CD133",  "GLAST","CD71") 

mat <- as.matrix(reference_data)
# rename genes to proteins
...
hm + ggtitle("Expression by cell type from RNA sequencing") +
  theme(plot.title = element_text(hjust = 0.5))

pdf("ReferenceMatrixRNAonly.pdf", width = 6.5, height = 5)
hm + ggtitle("Expression by cell type from RNA sequencing") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()
...
gene.names <- rownames(trans_df_RNA)
# see the two lists
gene.names
protein.names
...
rownames(trans_df_RNA) <- gene.protein
rownames(trans_df_RNA)
colnames(trans_df_RNA)
...
colnames(trans_df_RNA)
# change cell type names to match

colnames(fc) <-c("StemCell", "Neurons","OPC","Oligo","NPC","Astrocytes", "Endothelial","Epithelial", "RG")
rownames(trans_df_RNA)
fc <- fc %>% select("StemCell", "Neurons","OPC","Oligo","NPC","Astrocytes", "Endothelial","Epithelial", "RG")
...
# Get missing cell types in fc
missing_cell_types <- setdiff(colnames(trans_df_RNA), colnames(fc))

# Create a data frame with missing cell types and assign value 0.5 to all markers
missing_data <- data.frame(cell_type = missing_cell_types,
                           matrix(0.5, nrow = length(missing_cell_types), ncol = nrow(fc)),
                           row.names = NULL)

# Assign column names to the missing data frame
colnames(missing_data)[-1] <- rownames(fc)

# Bind the missing data to fc
# Transpose missing_data
missing_data <- t(missing_data)

if (nrow(missing_data) != nrow(fc)) {
  # If not, subset missing_data to match the rows of fc
  missing_data <- missing_data[rownames(fc), ]
}

# Bind the missing data to fc
fc <- cbind(fc, missing_data)
fc
class(fc)
# convert to numeric 

fc_temp <- fc
fc_temp[] <- lapply(fc, as.numeric)
fc_temp
...
# Reorder column and row names of fc_temp to match trans_df_RNA
fc_temp <- fc_temp[, colnames(trans_df_RNA)]
fc_temp <- fc_temp[row.names(trans_df_RNA), ]
library(tidyr)

# Convert fc_temp to data frame and set row names as a column
fc_df <- fc_temp

fc_df$markers <- rownames(fc_df)

# pivot long
fc_long <- pivot_longer(data = fc_df, 
                        cols = -markers,
                        names_to = "cell_type",
                        values_to = "value")

# Convert trans_df_RNA to long format
rna_df <- as.data.frame(trans_df_RNA)
rna_df$markers <- rownames(rna_df)
rna_long <- pivot_longer(data = rna_df, 
                        cols = -markers,
                        names_to = "cell_type",
                        values_to = "value")

rna_long
# Add marker column to rna_long

# merge 
combined_data <- left_join(fc_long, rna_long, by = c("markers", "cell_type"), suffix = c(".protein", ".rna"))

# Plot

# dot plot only
ggplot(combined_data, aes(x = value.protein, y = value.rna, color = markers)) +
  geom_point() +
  facet_wrap(~ cell_type, scales = "free_x") +
  labs(x = "Protein Value", y = "RNA Value", color = "Marker") +
  theme_minimal()


### show correlation

correlation <- cor(combined_data$value.protein, combined_data$value.rna)
...
ggplot(combined_data, aes(x = value.protein, y = value.rna, color = markers)) +
  ...
  facet_wrap(~ cell_type, scales = "free_x") +
  labs(x = "Protein Value", y = "RNA Value", color = "Marker", title = paste("Correlation Coefficient:", round(correlation, 2))) +
  theme_minimal()

# with the R value
# Plot with linear regression lines
gg <- ggplot(combined_data, aes(x = value.protein, y = value.rna, color = markers)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +  # Add linear regression line
  facet_wrap(~ cell_type, scales = "free_x") +
  labs(x = "Protein Value", y = "RNA Value", color = "Marker") +
...
gg <- ggplot(combined_data, aes(x = value.protein, y = value.rna, color = markers)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "grey", linetype = "dashed") +  # Add linear regression line
  facet_wrap(~ cell_type, scales = "free_x") +
  labs(x = "Protein Value", y = "RNA Value", color = "Marker") +
  theme_minimal()
...
gg <- gg +
  geom_text(data = correlation_data, aes(label = paste("R =", round(correlation, 2))), 
            x = -Inf, y = Inf, hjust = 0, vjust = 1, size = 3, color = "black", 
            inherit.aes = FALSE, show.legend = FALSE)
