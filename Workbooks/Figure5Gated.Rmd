...
sampleNames(flowset) <- c("Astrocytes","Endothelial","Epithelial","Neurons1",
                           "Neurons2", "NPC", "Oligodendrocytes","RadialGlia","StemCellLike")
sampleNames(flowset)
...
write.csv(df, "/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure5/FlowsetGatedCellPops.csv")


df2 <- flowset_to_csv(flowset) # not aligned
...
seu2 <- make_seu(df2, AB_vector = AB)
table(seu2$Sample)

Idents(seu2) <- "Sample"
seu2 <- subset(seu2, downsample = 10000)
...
```{r}
for (i in AB) {
  print(FeaturePlot(seu2, features = i, min.cutoff = 'q1', max.cutoff = 'q97', label = TRUE, raster = FALSE))
}
...
```{r}
library(data.table)
length(unique(seu2$seurat_clusters))
plotmean(plot_type = 'heatmap',seu = seu2, group = 'RNA_snn_res.1.2',
         markers = AB, 
               var_names = c(0:25), slot = 'scale.data', xlab = "Cluster",
               ylab = "Markers")
...
# because I downsampled after making the seurat object from the flowset I need to reexact the expression values
Idents(seu2) <- "Sample"
df.ex <- as.data.frame(t(as.data.frame(GetAssayData(seu2, slot = "counts"))))
head(df.ex)
#df.ex$X <- rownames(df.ex)
write.csv(df.ex, "expressionGatedCells.csv")

df_test <- read.csv("expressionGatedCells.csv") # the X column needs to be the first column
...
write.csv(cor, "GatedCellsCorPredict.csv")

plot_corr(cor, threshold = 0.35, min_cells = 100)
...
seu2 <- AddMetaData(object=seu2, metadata=cor$cell.label, col.name = 'cor.labels')

Idents(seu2) <- 'cor.labels'
library(dplyr)
# filter cell types less than 100 cells and filter out unassigned from the 0.35 threshold with 0.05 double cell threshold
thresh = 100
freq.cor <- data.frame(table(seu2$cor.labels))
levels(seu2)
...
seu.sub <- subset(seu2, ident = cell.order, invert = FALSE)
...
rfm.pred <- RFM_predict(seu2, rf)
head(rfm.pred)
unique(rfm.pred$Prediction)


# add the predictions to the seurat object
seu2 <- AddMetaData(object=seu2, metadata= as.factor(rfm.pred$Prediction), col.name = 'rfm.pred')



celltypes <- unique(seu2$rfm.pred)
sorted <- as.character(factor(sort(as.character(celltypes))))

DimPlot(seu2, group.by = 'rfm.pred', raster = FALSE, label = TRUE, order = rev(sorted))
...
plot_lab_clust(seu2, seu2$RNA_snn_res.1.2, seu2$rfm.pred, filter_out = c( "Unassigned","Mixed"))


 t.lables <- as.data.frame(table(seu2$RNA_snn_res.1.2, seu2$rfm.pred))
...
saveRDS(seu.q,"/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure4/All9MOannaoteAug.RDS")
