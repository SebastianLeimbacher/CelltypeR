...
DimPlot(seu.q, raster = FALSE, label = TRUE)
...
plotmean(plot_type = 'heatmap',seu = seu, group = 'RNA_snn_res.1.5',
         markers = AB, 
...
cor1 <- read.csv("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure4/AllcellsCor_thesh35.csv")

cor1 <- cor1 %>% select(-"X.1")
...
```{r, fig.width=5}
#seu.q <- AddMetaData(object=seu.q, metadata=cor1$cell.label, col.name = 'cor.labels1')
Idents(seu.q) <- "RNA_snn_res.1.5"
...
setwd("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure4/")
...
dev.off()
 ...
 pdf("BarchartCorbyClusterThresh035Filter.pdf", width = 9.5, height = 4)
...
Cor filter again but keep unassigned
...
```{r}
seu.q <- annotate(seu.q,annotations = cor.ann.035$CAM, to_label= "RNA_snn_res.1.5", annotation_name = "CAM")
...
rf <- readRDS("/Users/rhalenathomas/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/RFM/June9MBOsub_trainedRFM.Rds")
...
rfm.pred <- RFM_predict(seu.q, rf, markers = AB)
head(rfm.pred)
...
celltypes <- unique(seu.q$rf.pred)
sorted <- as.character(factor(sort(as.character(celltypes))))


DimPlot(seu.q, group.by = 'rf.pred', raster = FALSE, label = TRUE, order = rev(sorted))

# change the order of the predicted cells
Idents(seu.q) <- 'rf.pred'
seu.q <- AddMetaData(seu.q, metadata = seu.q$rf.pred, col.name = 'rfm.sort')
...
setwd("~/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure4")
 pdf("BarchartRFMbyClusterS.pdf", width = 8, height = 4)
 plot_lab_clust(seu.q, seu.q$clusters, seu.q$rfm.sort, filter_out = c( "Unassigned","Mixed"))
 dev.off()
 
png("UMAPRFMpredictionsLB.png", width = 800, height = 400)
DimPlot(seu.q, group.by = "rfm.pred", raster = FALSE,  label = TRUE, 
        repel = TRUE, label.size = 5.5,
        order = sorted) + theme_classic(base_size = 20)
 dev.off()
...
```{r}
 ...
 t.lables <- as.data.frame(table(seu.q$RNA_snn_res.1.5, seu.q$rf.pred))
  t.lables$Freq <- as.double(t.lables$Freq)
  colnames(t.lables) <- c("Cluster", "Label","Freq")
  top.labs <- t.lables  %>% group_by(Cluster) %>% top_n(5, Freq)
  sort.tops <- top.labs %>% as.data.frame() %>% arrange(desc(Freq))  %>% arrange(Cluster)
