...
Cell Ranger was run for HTO demultiplexing (Feature sequencing)
Seurat objects were created in a computer cluster (DRAC HPC server beluga). 

# In this notebook 
1. Processing each of the 4 FACS single cell sequencing datasets
2. Merging and annotating main cell types
3. Exploring DA neuron markers and subtype markers. 

Read in the samples

```{r}
pathway <- "/scRNAseqSorted/objs/"
...
DoHeatmap(seu.sc, features = PD_poulin, group.by = "Cell_Type2") + ggtitle("DA markers")
DotPlot(seu.sc, features = PD_poulin, group.by = 'Cell_Type2') + ggtitle("DA markers") +
  theme(axis.text.x = element_text(angle = 90))
DotPlot(seu.sc, features = "TH", group.by = 'Cell_Type2') + ggtitle("DA markers") +
  theme(axis.text.x = element_text(angle = 90))

# many cell types a few markers
ft <- c("COL3A1","FABP5","HPD","RAB3B","TPBG","TTR","SOX2","ASCL1","CP","GRIA2",
        "MGP","SPARCL1","TPH1","TFPI1","CY1B1","NEAT1","PTN","RPL41","TOP2A","VCAN")

DoHeatmap(seu.sc, features = ft, group.by = "Cell_Type2", slot = 'scale.data') + ggtitle("Multiple cell type Markers")
DotPlot(seu.sc, features = ft, group.by = "Cell_Type2") +
  theme(axis.text.x = element_text(angle = 90)) + ggtitle("Multiple cell type Markers")


# neurons and TH 
feature_list = c("MAP2","NCAM1","CD24","GRIA2","GRIN2B","GAD1",
                 "GAD2","GABRA1","GABRB2","TH","ALDH1A1","LMX1B",
                 "NR4A2","CORIN","CALB1","KCNJ6","CXCR4","ITGA6",
                 "SLC1A3","CD44","AQP4","S100B", "PDGFRA","OLIG2",
                 "MBP","CLDN11","VCAM1")

DoHeatmap(seu.sc, features = feature_list, group.by = "Cell_Type2", slot = 'scale.data') + ggtitle("Neuron and DA neuron and other cell type markers")
DotPlot(seu.sc, features = feature_list, group.by = "Cell_Type2") +
  theme(axis.text.x = element_text(angle = 90)) + ggtitle("Neuron and DA neuron markers and other cell type markers")


# neuron markers and TH 
feature_list <- c("RBFOX3","GRIN2B","GAD1","GAD2","GABRA1","GABRB2","TH")
DoHeatmap(seu.sc, features = feature_list, group.by = "Cell_Type2", slot = 'scale.data') + ggtitle("Neuron markers and TH")
DotPlot(seu.sc, features = feature_list, group.by = "Cell_Type2") +
  theme(axis.text.x = element_text(angle = 90)) + ggtitle("Neuron markers and TH")

# NPC, early neuron, Glia
feature_list <- c("SLC1A3", "PAX6", "SOX2", "PDGFD", "GLI3", "STMN2", "NEUROD6", "VIM", "HES1")
feature_list <- c("RBFOX3","GRIN2B","GAD1","GAD2","GABRA1","GABRB2","TH")
DoHeatmap(seu.sc, features = feature_list, group.by = "Cell_Type2", slot = 'scale.data') + ggtitle("NPC, early neuron, Glia")
DotPlot(seu.sc, features = feature_list, group.by = "Cell_Type2") +
  theme(axis.text.x = element_text(angle = 90)) + ggtitle("NPC, early neuron, Glia")
...
DoHeatmap(seu.sc, features = feature_list, group.by = "Cell_Type2", slot = 'scale.data') + ggtitle("Cell type Markers")
DotPlot(seu.sc, features = feature_list, group.by = "Cell_Type2") +
  theme(axis.text.x = element_text(angle = 90)) + ggtitle("Cell type Markers")
...
DotPlot(seu.sc, features = feature_list.d, group.by = 'Cell_Subtype_Markers') +
  theme(axis.text.x = element_text(angle = 90))+ ggtitle("Subtype Markers")
...
Markers from Poulin Review (2014)
...
Early vs mature DA markers and iPSC markers
...
```
Aguila identify stable DGE distinguishing human VTA vs SN
