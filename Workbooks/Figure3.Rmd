...
seu <- annotate(seu, annotations = cluster.ids, to_label = "RNA_snn_res.0.7",
...
setwd("~/Documents/Data/FlowCytometry/PhenoID/Analysis/NatMethodJuneSubmission/Figure3/")
...
Idents(seu.r) <- "labels"
rfm.pred.ref <- RFM_predict(seu.r, rf2, markers)
head(rfm.pred.ref)
unique(rfm.pred.ref)
table(rfm.pred.ref)

seu.r <- AddMetaData(object=seu.r, metadata=rfm.pred.ref$Prediction, col.name = 'rfm.labels')
dftrue <- as.data.frame(table(as.character(seu.r$labels)))
dfpred <- as.data.frame(table(as.character(seu.r$rfm.labels)))
colnames(dfpred)
df2 <- dfpred %>% 
   add_row(Var1 = "Glial-lineage", Freq=0)
df2
df_merge <- merge(dftrue, df2, by = "Var1", all = TRUE)
colnames(df_merge) <- c("Celltype","Truth","Prediction")
df_merge$Precision <- df_merge$Prediction / df_merge$Truth
...
# confusion matrix 
table_by_celltype <- table(Prediction = seu.r$rfm.labels, Truth = seu.r$labels)
df_table <- as.data.frame(table_by_celltype)


heatmap(table_by_celltype, Rowv = NA, Colv = NA)
...
```{r}
# check the predictions
Idents(seu.sub) <- "labels"
rfm.pred.ref <- RFM_predict(seu.sub, rf2, markers)
table(rfm.pred.ref)
seu.sub <- AddMetaData(object=seu.sub, metadata=rfm.pred.ref$Prediction, col.name = 'rfm.labels')
dftrue <- as.data.frame(table(as.character(seu.sub$labels)))
dfpred <- as.data.frame(table(as.character(seu.sub$rfm.labels)))

df_merge <- merge(dftrue, dfpred, by = "Var1", all = TRUE)
colnames(df_merge) <- c("Celltype","Truth","Prediction")
df_merge$Precision <- df_merge$Prediction / df_merge$Truth
...
# confusion matrix 
table_by_celltype <- table(Prediction = seu.sub$rfm.labels, Truth = seu.sub$labels)
df_table <- as.data.frame(table_by_celltype)
...
saveRDS(rf2, "TrainedRFMAug10_noglialineage.Rds")
