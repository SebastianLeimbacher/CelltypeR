...
pdf("HM9000_fig3EMarch.pdf",width = 8.5, height = 5)
DoHeatmap(seu, features = AB, size= 6,slot = "scale.data", group.colors = clust.colours, disp.max = 1, disp.min = -1,
          angle = 90) + scale_fill_gradientn(colors = c("#0b41b5","#5ecc58","#faec70")) + 
  theme(axis.text.y = element_text(size = 15)) # low, med, high
dev.off()

### change colours in the heatmap

DoHeatmap(seu, features = AB, size= 6,slot = "scale.data", group.colors = clust.colours, disp.max = 2, disp.min = -1.5,
          angle = 90) + scale_fill_gradientn(colors = c("#440144FF", "#22A884FF","#FDE725FF")) + 
  theme(axis.text.y = element_text(size = 15)) # low, med, high


DoHeatmap(seu, features = AB, size= 6,slot = "scale.data", group.colors = clust.colours, disp.max = 1, disp.min = -1,
          angle = 90) + scale_fill_gradientn(colors = c("#403891b2", "#de7065b2","#f7cb44b2")) + 
  theme(axis.text.y = element_text(size = 15)) # low, med, high



DoHeatmap(seu, features = AB, size= 6,slot = "scale.data", group.colors = clust.colours, disp.max = 1, disp.min = -1,
          angle = 90) + scale_fill_gradientn(colors = c("#12206dff", "#de7065ff","#f9b641ff")) + 
  theme(axis.text.y = element_text(size = 15)) # low, med, high


DoHeatmap(seu, features = AB, size= 6,slot = "scale.data", group.colors = clust.colours, disp.max = 2, disp.min = -1,
          angle = 90) + scale_fill_gradientn(colors = c("#0b41b5","#5ecc58","#faec70")) + 
  theme(axis.text.y = element_text(size = 15)) # low, med, high
